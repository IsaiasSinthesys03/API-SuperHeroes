 ...
 *   get:
 *     summary: Estado de vida del personaje 3 del Jugador 1
 *     responses:
 *       200:
 ...
 *   get:
 *     summary: Acciones del Jugador 1 en Round 3
 *     responses:
 *       200:
 ...
 *   post:
 *     summary: Atacar en Round 3 (Jugador 1)
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
router.post('/round3jugador1/atacar', async (req, res) => {
  try {
    ...
    for (let i = acciones.length - 1; i >= 0; i--) {
      if (acciones[i].ID_Equipo1 === enf.ID_Equipo1 && acciones[i].jugador === 1 && acciones[i].AccionRound3 === 'Usar habilidad') {
    ...
    for (let i = acciones.length - 1; i > lastHabilidadIndex; i--) {
      if (acciones[i].ID_Equipo1 === enf.ID_Equipo1 && acciones[i].jugador === 1 && acciones[i].AccionRound3 === 'Golpear') {
    ...
    let golpeIndex = acciones.filter(a => a.ID_Equipo1 === enf.ID_Equipo1 && a.jugador === 1 && a.AccionRound3 === 'Golpear').length % 3;
    let danoBase = 0;
    let mensaje = '';
    ...
    if (enf.VidaPersonaje2_3 === 0) {
      await registrarGanadorRound3(1);
