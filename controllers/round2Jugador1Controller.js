...
router.get('/estados-vida', async (req, res) => {
  try {
    ...
    if (enf.VidaPersonaje1_2 === 0 || enf.VidaPersonaje2_2 === 0) {
      ...
      if (enf.VidaPersonaje1_2 === 0 && enf.VidaPersonaje2_2 > 0) {
        round2Winner = 'Jugador 2';
      } else if (enf.VidaPersonaje2_2 === 0 && enf.VidaPersonaje1_2 > 0) {
        round2Winner = 'Jugador 1';
      } else if (enf.VidaPersonaje1_2 === 0 && enf.VidaPersonaje2_2 === 0) {
        // Empate, ambos llegaron a 0
        round2Winner = 'Empate';
      }
...
router.post('/atacar', async (req, res) => {
  try {
    ...
    const accionesJugador1 = acciones.filter(a => a.jugador === 1 && a.ID_Equipo2 === enf.ID_Equipo2);
    ...
    // Restricción de habilidad: solo cada 3 golpes básicos del jugador 1
    let lastHabilidadIndex = -1;
    for (let i = accionesJugador1.length - 1; i >= 0; i--) {
      if (accionesJugador1[i].AccionRound2 === 'Usar habilidad') {
    ...
    for (let i = accionesJugador1.length - 1; i > lastHabilidadIndex; i--) {
      if (accionesJugador1[i].AccionRound2 === 'Golpear') {
    ...
    let golpeIndex = accionesJugador1.filter(a => a.AccionRound2 === 'Golpear').length % 3;
    let danoBase = 0;
    let mensaje = '';
    ...
    if (enf.VidaPersonaje2_2 === 0) {
      ...
      let round1 = peleaActual?.Round1 || null;
      let round3 = peleaActual?.Round3 || null;
      let ganador = calcularGanador(round1, 'Jugador 1', round3);
      await peleasService.default.registrarRound({
        id: enf.id,
        Round1: round1,
        Round2: 'Jugador 1',
